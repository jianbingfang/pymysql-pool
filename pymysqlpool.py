...
"""
filename: pymysql_pool.py
description: a more robust version of pymysql-pool (https://github.com/jkklee/pymysql-pool)
             with connection health check.
author: jianbingfang@qq.com
"""
...
class Connection(pymysql.connections.Connection):
  ...
  def __exit__(self, exc, value, traceback):
    ...
    if self._pool:
      if not exc or exc in self._reusable_expection:
        ...
        self._pool.put_connection(self._recreate())
        self._pool = None
        try:
          self.close()
  ...
  def _recreate(self):
    """ recreate new connection with the same args """
    logging.debug('Try creating new connection due to pool(%s) lacking', self._pool.name)
    conn = Connection(*self.args, **self.kwargs)
    conn._pool = self._pool
    logging.debug('New connection created and linked to pool(%s)', self._pool.name)
    return conn
  ...
  def __init__(self, size=5, name=None, *args, **kwargs):
    ...
    for _ in range(size if size < self._HARD_LIMIT else self._HARD_LIMIT):
      try:
        ...
        logging.error('Failed to init db connection!')
        self._pool.put(None)
  ...
  def get_connection(self, timeout=1, retry_num=1):
    ...
    try:
      ...
      try:
        ...
        logging.debug("The connection from pool(%s) is valid", self.name)
        return conn
      except Exception:
        # the exception could be caused by conn == None or Exception from conn.ping()
        raise DbConnectException
    except queue.Empty:
      if retry_num > 0:
        self._THREAD_LOCAL.retry_counter += 1
        logging.debug('Retry get connection from pool(%s), the %d times', self.name, self._THREAD_LOCAL.retry_counter)
        retry_num -= 1
      ...
      logging.info("The connection from pool(%s) is invalid, trying a new connection...", self.name)
      try:
        conn = Connection(*self.args, **self.kwargs)
        conn._pool = self
        return conn
